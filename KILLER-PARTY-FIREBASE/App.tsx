...
import { DEFAULT_MISSIONS, AVATARS, MODE_MISSIONS, MODE_LABELS, MODE_DESCRIPTIONS } from './constants.tsx';
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-4 gap-2">
        {altRow.map(k => (
          <button key={k} onClick={() => handlePress(k)} className="h-12 rounded-2xl bg-slate-900/60 border border-white/5 font-display font-bold text-base active:bg-rose-600 active:scale-95 transition-all">
            {k}
          </button>
        ))}
        <button onClick={() => handlePress('⌫')} className="h-12 rounded-2xl bg-slate-900/60 border border-white/5 font-display font-bold text-base active:bg-rose-600 active:scale-95 transition-all">
          ⌫
        </button>
        <button onClick={() => handlePress('CLR')} className="h-12 rounded-2xl bg-slate-900/60 border border-white/5 font-display font-bold text-base active:bg-rose-600 active:scale-95 transition-all">
          CLR
        </button>
        <button onClick={() => handlePress('✓')} className="h-12 rounded-2xl bg-rose-600/80 border border-rose-500/60 font-display font-bold text-base active:scale-95 transition-all">
          ✓
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  // 🆕 États pour la fonctionnalité de suspicion
      ...
      if (game.config.useAI) {
        try {
          const aiMissions = await generateMissions(
            ...
            `Soirée entre amis (mode: ${MODE_LABELS[mode]}). Groupe de 18 amis dans une maison.`,
            game.config.difficulty
          );
          missions = aiMissions.length > 0 ? aiMissions : (MODE_MISSIONS[mode] || DEFAULT_MISSIONS);
        } catch (error) {
          console.error('Erreur génération IA, utilisation des missions par défaut');
          missions = MODE_MISSIONS[mode] || DEFAULT_MISSIONS;
        }
      } else {
        missions = MODE_MISSIONS[mode] || DEFAULT_MISSIONS;
      }
        ...
        <div className="w-full max-w-md space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
          ...
          {(joinMode === 'host' || joinMode === 'join') && (
            <Card className="space-y-6">
              ...
              {joinMode === 'join' && (
                <div className="space-y-2">
                  ...
                  <div className="space-y-4">
                    ...
                    />
                    <PinKeypad value={pinCode} onChange={(val) => setPinCode(normalizeCode(val))} onComplete={handleJoinGame} />
                  </div>
                </div>
              )}
              ...
              {joinMode === 'host' && (
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">Mode de jeu</label>
