...
export default function App() {
  ...
  useEffect(() => {
    if (user) {
      ...
      localStorage.setItem('killer_game_id', game?.id || '');
    }
  }, [user, game]);
  ...
  useEffect(() => {
    ...
    if (savedUser && savedGameId) {
      try {
        const parsedUser = JSON.parse(savedUser);
        setUser(parsedUser);
        // La subscription Firebase chargera le jeu
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const gameId = game?.id || localStorage.getItem('killer_game_id');
    if (!gameId) return;
  ...
  const executeKill = async () => {
    ...
    try {
      ...
      setKillLogs([...killLogs, killEvent]);
