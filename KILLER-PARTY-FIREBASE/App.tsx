...
// Composant PinKeypad retiré - utilisation du clavier natif uniquement
...
export default function App() {
  ...
  const [showSettingsModal, setShowSettingsModal] = useState(false); // 🆕 Menu de paramètres avant jeu
  const [language, setLanguage] = useState<Language>('fr'); // 🆕 Langue du jeu
  
  // 🆕 États pour la fonctionnalité de suspicioné de suspicion
  ...
  // 🆕 Fonction pour mélanger et assurer la variété des missions
  // Garder une trace des missions utilisées pour éviter les répétitions
  ...
  const getVariedMissions = useCallback((modeType: 'FUN' | 'FUN_CORPORATE' | 'CHILL', count: number): Mission[] => {
    // Utiliser getRandomMissions de constants.tsx qui gère la variété
    const missionDescriptions = getRandomMissions(modeType, count, usedMissions);
    
    // Mettre à jour les missions utilisées
    ...
    // Retourner les missions formatées
  ...
  const startSaison = async () => {
    ...
    try {
      ...
      const missionCount = game.players.length * 2; // 2 missions par joueur pour plus de variété
      
      if (game.config.useAI) {
        try {
          const aiMissions = await generateMissions(
            missionCount,
            `Soirée entre amis (mode: ${MODE_LABELS[mode]}). Groupe de ${game.players.length} amis dans une maison.`,
        ...
        // 🆕 Utilisation de getVariedMissions pour assurer l'unicité
        missions = getVariedMissions(mode, missionCount);
      }

      // Assignation circulaire des cibles avec mélange des missions
      const shuffledPlayers = [...game.players].sort(() => Math.random() - 0.5);
      const shuffledMissions = [...missions].sort(() => Math.random() - 0.5); // 🆕 Mélange supplémentaire
      ...
      const updatedPlayers = shuffledPlayers.map((player, index) => {
        ...
        return {
          ...
          mission: shuffledMissions[missionIndex],
          status: PlayerStatus.ALIVE
        };
      });
  ...
  const executeKill = async () => {
    ...
    if (game.dispute?.status === 'pending') {
      setError("Une validation est déjà en attente.");
      return;
    }
  ...
  const resolveDispute = async (approved: boolean) => {
    ...
    if (!approved) {
      ...
      setTimeout(() => {
        updateGame(game.id, { dispute: undefined });
      }, 300);
      return;
    }
    ...
    const killEvent: KillEvent = {
      ...
      victimId: victim.id,
      timestamp: Date.now(),
      missionDesc: dispute.missionDesc || ''
    };
    ...
    const updatedPlayers = game.players.map(p => {
      if (p.id === killer.id) {
        return {
          ...
          targetId: victimTargetId,
          killCount: p.killCount + 1,
          score: p.score + (killer.mission?.difficulty || 1) * 10
        };
      }
      if (p.id === victim.id) {
        return {
    ...
    await updateGame(game.id, {
      ...
      killLogs: updatedKillLogs,
      status: newStatus,
      dispute: { ...dispute, status: 'validated' }
    });
  ...
  const handleSuspicion = async (accusedId: string) => {
    ...
    // Trouver qui est réellement le chasseur du joueur actuel
    ...
    // Créer l'événement de suspicion
    const suspicionEvent: Suspicion = {
      id: `s-${Date.now()}`,
      suspectId: currentPlayer.id,
      accusedId: accusedId,
      timestamp: Date.now(),
      isCorrect: isCorrect,
    ...
    try {
      await updateGame(game.id, {
        players: updatedPlayers,
        suspicions: [...(game.suspicions || []), suspicionEvent]
      });
      ...
      console.log('✅ Suspicion enregistrée');
    } catch (error) {
  ...
  const generateRecapImage = async () => {
    ...
    ctx.fillText('KILLER PARTY', 80, 140);

    ctx.fillStyle = '#e2e8f0';
    ctx.font = 'bold 32px Inter, sans-serif';
    ctx.fillText(`Partie: ${game.name}`, 80, 210);

    ctx.fillStyle = '#94a3b8';
    ctx.font = '24px Inter, sans-serif';
    ctx.fillText(new Date().toLocaleString('fr-FR'), 80, 255);

    const winner = [...game.players].sort((a, b) => b.score - a.score)[0];
    ctx.fillStyle = '#f8fafc';
    ctx.font = 'bold 40px Space Grotesk, sans-serif';
    ctx.fillText(`🏆 Vainqueur: ${winner?.name || '—'}`, 80, 340);
    ...
    ctx.fillText('Classement', 80, 570);
    ...
    topPlayers.forEach((p, index) => {
      ...
      ctx.fillStyle = '#e2e8f0';
      ctx.fillText(`${p.name}`, 130, y);
      ctx.fillStyle = '#94a3b8';
      ctx.fillText(`${p.score} pts`, 820, y);
    });
  ...
  const downloadRecapImage = () => {
    ...
    link.href = recapImageUrl;
    link.download = `killer-party-${game?.id || 'recap'}.jpg`;
    link.click();
  };
  ...
  const shareRecapImage = async () => {
    if (!recapImageUrl) return;
    try {
      const res = await fetch(recapImageUrl);
      ...
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'Killer Party',
          text: `Récap de partie ${game?.id || ''} !`
        });
      } else {
        downloadRecapImage();
      }
    } catch (error) {
      console.error('Erreur partage récap:', error);
      downloadRecapImage();
    }
  };
  ...
  useEffect(() => {
    ...
    if (profileId) {
      appendHistory(profileId, entry).catch((err) => {
                    ...
                    {(['FUN', 'FUN_CORPORATE', 'CHILL'] as const).map((mode) => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${
                          gameMode === mode ? 'border-rose-500/60 bg-rose-500/10' : 'border-white/5 bg-slate-900/40'
                        }`}
                      >
        ...
        <div className="w-full max-w-md space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-slate-400">
              Votre partie en cours n'est plus disponible. Vous pouvez repartir de zéro.
            </p>
          </div>
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans select-none overflow-hidden">
      ...
      {/* 🆕 Overlay de résultat de suspicion */}
      {suspicionResult?.show && (
        <div className="fixed inset-0 z-[200] bg-slate-950/90 backdrop-blur-2xl flex flex-col items-center justify-center animate-in fade-in duration-300">
          ...
          <h2 className={`text-4xl font-black italic font-display uppercase tracking-tighter mb-4 ${
            suspicionResult.correct ? 'text-green-500' : 'text-rose-500'
          }`}>
            {suspicionResult.correct ? 'SUSPICION CORRECTE !' : 'SUSPICION INCORRECTE'}
          </h2>
          <p className="text-2xl font-bold">
            {suspicionResult.points > 0 ? '+' : ''}{suspicionResult.points} points
          </p>
        </div>
      )}
          ...
          <div className="space-y-8">
            {pendingDispute && (
              <Card className="border-rose-500/40 bg-rose-500/10">
                <p className="text-sm text-rose-300 font-bold text-center">
                  Validation en attente de la cible…
                </p>
              </Card>
            )}
            {activeTab === 'secret' && (
              <div className="space-y-6">
                {currentPlayer?.status === PlayerStatus.ALIVE ? (
                        ...
                        {isRevealed ? (
                          <div className="flex items-center gap-6 animate-in zoom-in-95">
                            ...
                            <div className="text-left">
                              ...
                              <p className="text-[10px] font-black text-slate-500">CIBLE</p>
                            </div>
                          </div>
                        ) : (
                  ...
                  <div className="py-20 text-center">
                    ...
                    <p className="text-slate-500 text-xs mt-2 uppercase font-black">Joueur éliminé</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeTab === 'feed' && (
              <div className="space-y-4">
                ...
                {/* 🆕 Afficher les suspicions */}
                {(game.suspicions || []).length > 0 && (
                  <div className="space-y-2">
                    <h4 className="text-sm font-black uppercase tracking-wider text-slate-500">Suspicions</h4>
                ...
                {killLogs.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    <h4 className="text-sm font-black uppercase tracking-wider text-slate-500">Éliminations</h4>
            ...
            {activeTab === 'players' && (
              <div className="space-y-4">
                <h3 className="text-2xl font-black italic font-display uppercase">👥 Joueurs</h3>
                <Card className="space-y-4">
                  <div className="flex items-center gap-4">
      ...
      {game?.status === GameStatus.ACTIVE && (
        <nav className="fixed bottom-0 left-0 right-0 bg-slate-950/80 border-t border-white/5 backdrop-blur-3xl px-10 pb-10 pt-5 z-40">
          <div className="max-w-md mx-auto flex items-center justify-between">
            ...
            <NavBtn label="Joueurs" icon="👥" active={activeTab === 'players'} onClick={() => setActiveTab('players')} />
          </div>
        </nav>
      )}
      ...
      {killConfirmModal && (
        <div className="fixed inset-0 z-[150] bg-slate-950/90 backdrop-blur-2xl flex items-center justify-center p-8">
          <Card className="w-full max-w-sm space-y-6 border-rose-500/50" glow>
            <h3 className="text-xl font-black italic text-rose-500 uppercase text-center">Confirmation d'élimination</h3>
        ...
        <div className="fixed inset-0 z-[150] bg-slate-950/90 backdrop-blur-2xl flex items-center justify-center p-8 overflow-y-auto">
          <Card className="w-full max-w-sm space-y-6 border-rose-500/50" glow>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto">
              {game?.players
                .filter(p => p.status === PlayerStatus.ALIVE && p.id !== currentPlayer?.id)
                .map(p => (
                  <button
                    key={p.id}
                    onClick={() => handleSuspicion(p.id)}
            ...
            <Button variant="outline" onClick={() => setSuspicionModal(false)} className="w-full">
              ANNULER
            </Button>
          </Card>
        </div>
      ...
      {game?.dispute?.status === 'pending' && currentPlayer?.id === game.dispute.victimId && (
        <div className="fixed inset-0 z-[160] bg-slate-950/90 backdrop-blur-2xl flex items-center justify-center p-8">
          <Card className="w-full max-w-sm space-y-6 border-rose-500/50" glow>
            ...
            <p className="text-xs text-slate-400 text-center uppercase tracking-wider">
              Le joueur <span className="text-white font-black">
                {game.players.find(p => p.id === game.dispute?.killerId)?.name}
              </span> confirme vous avoir éliminé.
            </p>
            <div className="space-y-3">
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[170] bg-slate-950/90 backdrop-blur-2xl flex items-center justify-center p-8">
          <Card className="w-full max-w-sm space-y-4 border-rose-500/40" glow>
            <h3 className="text-xl font-black italic text-rose-500 uppercase text-center">Paramètres</h3>
      ...
      {/* 🆕 Modal de paramètres avant jeu (langue et mode) */}
      {showSettingsModal && (
        <div className="fixed inset-0 z-[180] bg-slate-950/95 backdrop-blur-2xl flex items-center justify-center p-8 overflow-y-auto">
          <Card className="w-full max-w-md space-y-6 border-rose-500/40" glow>
              ...
              <div className="grid grid-cols-2 gap-2">
                {(Object.keys(LANGUAGES) as Language[]).map((lang) => (
                  <button
                    ...
                    onClick={() => {
                      setLanguage(lang);
                      localStorage.setItem('killer_language', lang);
                    }}
                    className={`w-full text-left px-4 py-3 rounded-xl border transition-all ${
                      language === lang ? 'border-rose-500/60 bg-rose-500/10' : 'border-white/5 bg-slate-900/40'
                    }`}
                  >
            ...
            {/* Sélection du mode de jeu par défaut */}
            <div className="space-y-3">
              <label className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-500">{t('game_mode')}</label>
              <div className="grid grid-cols-1 gap-2">
                {(['FUN', 'FUN_CORPORATE', 'CHILL'] as const).map((mode) => (
                  <button
                    key={mode}
                  ...
                  >
                    <div className="text-sm font-bold text-white">{MODE_LABELS[mode]}</div>
                    <div className="text-[10px] text-slate-500">{MODE_DESCRIPTIONS[mode]}</div>
                  </button>
                ))}
              </div>
            </div>
